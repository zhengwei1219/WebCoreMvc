...
namespace Workflows.DAO
{
	...
	{
        NHStateMachineInstanceDao dao;
        NHActivityDao _nHActivityDao;
        public NHWorkflowPersistService(NHStateMachineInstanceDao instanceDao)
        {
            dao = instanceDao;
        }
		/// <summary>
		...
		protected Activity GetApprovalActivity(long id)
		{
			...
			NHActivity activity = _nHActivityDao.GetById(id, false);
			return activity.CovertTo();
		}

		protected List<Activity> GetApprovalActivitys(long[] ids)
		{
			List<NHActivity> lists = _nHActivityDao.GetNHActivitys(ids);
